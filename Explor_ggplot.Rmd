...
ggplot(airpoll2, aes(x=ind, y= log(values+1)))+
  geom_jitter( aes(color=ind),width = 0.2, alpha= 0.8)+
  geom_violin(aes(color=ind),
              scale = "width", 
              alpha= 0.1, # valor de opacidad
              size= 1)+ # tamaño de la linea
  geom_boxplot(width = 0.3, alpha= 0.1, size= 1,
               outlier.color = "red",
               outlier.alpha = 0.9,
               outlier.size = 2)+
...
airpoll4 <- airpoll %>% 
  rownames_to_column(var= "City") %>% 
  pivot_longer(cols = c(names(airpoll)),
               names_to = "variable",
               values_to = "valores")
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(airpoll4)
```
